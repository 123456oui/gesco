...
<div class="container-fluid">
    <div class="main-card card">
        <div class="card-header py-0 position-relative text-center">
            <h4 class="w-100 m-0">{{ __('Liste des élèves Noël') }}</h4>

            {{-- Bouton imprimer aligné à droite --}}
            <form action="{{ route('lnoel.imprimer') }}" method="POST" style="position: absolute; right: 15px; top: 50%; transform: translateY(-50%);">
                @csrf
        ...
        <div class="card-body table-responsive">
            ...
            <p class="text-muted text-right mx-5">
                Total Noël : {{ number_format($reglementsParMatricule->sum('montant_noel'), 0, ',', ' ') }} F
            </p>
        </div>
    </div>
</div>
